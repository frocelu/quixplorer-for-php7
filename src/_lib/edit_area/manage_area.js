		...
		else if(this.textareaFocused && editAreas[this.id]["displayed"]==true && this.isResizing==false)
		{
			...
			if(this.last_selection["line_start"] != infos["line_start"] || this.last_selection["line_nb"] != infos["line_nb"] || infos["full_text"] != this.last_selection["full_text"] || this.reload_highlight || this.last_selection["selectionStart"] != infos["selectionStart"] || this.last_selection["selectionEnd"] != infos["selectionEnd"] || !timer_checkup )
			{
				...
				if(this.do_highlight==true)
				{
					...
					//curr_text[start]= curr_text[start].mb_substr(0,infos["curr_pos"]-1) +"¤_overline_¤"+ curr_text[start].mb_substr(infos["curr_pos"]-1);
					for(i=start; i< end; i++){
						content+= curr_text[i]+"\n";
					}
					...
					content		= content.mb_substr( 0, infos["curr_pos"] - 1 ) + "\r\r" + content.mb_substr( infos["curr_pos"] - 1, selLength ) + "\r\r" + content.mb_substr( infos["curr_pos"] - 1 + selLength );
	...
	EditArea.prototype.get_selection_infos= function(){
		...
		if(start>0){
			str					= sel["full_text"].mb_substr(0,start);
		...
		if(end>start){
			sel["line_nb"]=sel["full_text"].mb_substring(start,end).split("\n").length;
		}
	...
	EditArea.prototype.getIESelection= function(){
		...
		try{
			...
			selectionStart	+= ( line_start - this.textarea.value.mb_substr(0, selectionStart).split("\n").length)*2;		// count missing empty \r to the selection
			selectionStart	-= ( line_start - this.textarea.value.mb_substr(0, selectionStart).split("\n").length ) * 2;

			selectionEnd	= selectionStart + range.text.length;
			selectionEnd	+= (line_start + line_nb - 1 - this.textarea.value.mb_substr(0, selectionEnd ).split("\n").length)*2;
	...
	EditArea.prototype.setIESelection= function(){
		...
		nbLineStart	= a.value.mb_substr(0, a.selectionStart).split("\n").length - 1;
		nbLineEnd 	= a.value.mb_substr(0, a.selectionEnd).split("\n").length - 1;
		range		= document.selection.createRange();
	...
	EditArea.prototype.checkTextEvolution=function(lastText,newText){
		...
		while( cpt<end && step>=1 ){
            if(lastText.mb_substr(cpt, step) == newText.mb_substr(cpt, step)){
                cpt+= step;
            }else{
		...
		ch.posStart	= cpt;
		ch.lineStart= newText.mb_substr(0, ch.posStart).split("\n").length -1;
		...
		while( cpt>=0 && cpt_last>=0 && step>=1 ){
            if(lastText.mb_substr(cpt_last-step, step) == newText.mb_substr(cpt-step, step)){
                cpt-= step;
                cpt_last-= step;
            }else{
                step= Math.floor(step/2);
		...
		ch.newText		= newText.mb_substring(ch.posStart, ch.posNewEnd);
		ch.lastText		= lastText.mb_substring(ch.posStart, ch.posLastEnd);

		ch.lineNewEnd	= newText.mb_substr(0, ch.posNewEnd).split("\n").length -1;
		ch.lineLastEnd	= lastText.mb_substr(0, ch.posLastEnd).split("\n").length -1;
	...
	EditArea.prototype.tab_selection= function(){
		...
		var insText = this.textarea.value.mb_substring(start, end);
		...
		if (insText.length == 0) {
			...
			this.textarea.value = this.textarea.value.mb_substr(0, start) + this.tabulation + this.textarea.value.mb_substr(end);
			pos_start = start + this.tabulation.length;
			pos_end=pos_start;
		} else {
			start= Math.max(0, this.textarea.value.mb_substr(0, start).lastIndexOf("\n")+1);
			endText=this.textarea.value.mb_substr(end);
			startText=this.textarea.value.mb_substr(0, start);
			tmp= this.textarea.value.mb_substring(start, end).split("\n");
	...
	EditArea.prototype.invert_tab_selection= function(){
		...
		var insText	= a.value.mb_substring(start, end);

		/* Tab remove and cursor seleciton adjust */
		var pos_start=start;
		var pos_end=end;
		if (insText.length == 0) {
			if(a.value.mb_substring(start-t.tabulation.length, start)==t.tabulation)
			{
				a.value		= a.value.mb_substr(0, start-t.tabulation.length) + a.value.mb_substr(end);
				pos_start	= Math.max(0, start-t.tabulation.length);
				pos_end		= pos_start;
			}
			/*
			a.value = a.value.mb_substr(0, start) + t.tabulation + insText + a.value.mb_substr(end);
			pos_start = start + t.tabulation.length;
			pos_end=pos_start;*/
		} else {
			start		= a.value.mb_substr(0, start).lastIndexOf("\n")+1;
			endText		= a.value.mb_substr(end);
			startText	= a.value.mb_substr(0, start);
			tmp			= a.value.mb_substring(start, end).split("\n");
			insText		= "";
			for(i=0; i<tmp.length; i++){
				for(j=0; j<t.tab_nb_char; j++){
					if(tmp[i].charAt(0)=="\t"){
						tmp[i]=tmp[i].mb_substr(1);
						j=t.tab_nb_char;
	...
	EditArea.prototype.press_enter= function(){
		...
		var start=this.textarea.selectionStart;
		var end= this.textarea.selectionEnd;
		var start_last_line= Math.max(0 , this.textarea.value.mb_substring(0, start).lastIndexOf("\n") + 1 );
		var begin_line= this.textarea.value.mb_substring(start_last_line, start).replace(/^([ \t]*).*/gm, "$1");
		var lineStart = this.textarea.value.mb_substring(0, start).split("\n").length;
		...
		//alert(start_last_line+" strat: "+start +"\n"+this.textarea.value.mb_substring(start_last_line, start)+"\n_"+begin_line+"_")
		this.textarea.value= this.textarea.value.mb_substring(0, start) + begin_line + this.textarea.value.mb_substring(end);
	...
	EditArea.prototype.findEndBracket= function(infos, bracket){
		...
		if(end==-1)
			return false;
		var endLastLine=infos["full_text"].mb_substr(0, end).lastIndexOf("\n");
		if(endLastLine==-1)
			line=1;
		else
			line= infos["full_text"].mb_substr(0, endLastLine).split("\n").length + 1;

		var curPos= end - endLastLine - 1;
		var endLineLength	= infos["full_text"].mb_substring(end).split("\n")[0].length;
		this.displayToCursorPosition("end_bracket", line, curPos, infos["full_text"].mb_substring(endLastLine +1, end + endLineLength));
		return true;
	};
	...
	EditArea.prototype.displayToCursorPosition= function(id, start_line, cur_pos, lineContent, no_real_move){
		...
		dest		= _$(id);
		content		= "<span id='test_font_size_inner'>"+lineContent.mb_substr(0, cur_pos).replace(/&/g,"&amp;").replace(/</g,"&lt;")+"</span><span id='endTestFont'>"+lineContent.mb_substr(cur_pos).replace(/&/g,"&amp;").replace(/</g,"&lt;")+"</span>";
		if( this.isIE || ( this.isOpera && this.isOpera < 9.6 ) ) {
	...
	EditArea.prototype.area_get_selection= function(){
		...
		if( document.selection ){
			...
			text= this.textarea.value.mb_substring(this.textarea.selectionStart, this.textarea.selectionEnd);
		}
		return text;
	};
