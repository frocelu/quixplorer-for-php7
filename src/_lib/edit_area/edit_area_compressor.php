	...
	class Compressor{
			...
			if (($dir = @opendir($reg_path)) !== false)
			{
				while (($file = readdir($dir)) !== false)
				{
					if( $file !== "." && $file !== ".." && ( $pos = strpos( $file, '.js' ) ) !== false )
					{
						...
						{
							$a_displayName[] = "'". mb_substr( $file, 0, $pos ) ."':'". htmlspecialchars( $match[3], ENT_QUOTES ) ."'";
						}
					}
				}
				closedir($dir);
		...
		function send_datas()
		{
			if($this->param['debug']){
				...
				$header.=sprintf("javascript size: based files: %s => PHP COMPRESSION => %s ", $this->file_loaded_size, mb_strlen($this->datas));
				if($this->use_gzip){
					$gzip_datas=  gzencode($this->datas, 9, FORCE_GZIP);
					$header.=sprintf("=> GZIP COMPRESSION => %s", mb_strlen($gzip_datas));
					$ratio = round(100 - mb_strlen($gzip_datas) / $this->file_loaded_size * 100.0);
				}else{
					$ratio = round(100 - mb_strlen($this->datas) / $this->file_loaded_size * 100.0);
				}
...
//			die;
		...
		function file_get_contents($file)
		{
			...
			$content = fread($fd, filesize($file));
			fclose($fd);
			$this->file_loaded_size+= mb_strlen($content);
			return $content;
		}
