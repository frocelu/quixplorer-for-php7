 ...
 ***/
		...
		if(t && t.focused)
		{
			...
			try
			{
				...
				//	alert("rangeoffset: "+ range.offsetTop +"\ncalcoffsetTop: "+ calculeOffsetTop(t) +"\nrelativeTop: "+ relative_top);
					...
					tab	= t.value.mb_substr(0, range_start).split("\n");
					...
					tab	= t.value.mb_substr(0, range_start + range.text.length).split("\n");
	...
	function set_IE_selection( t ){
		...
		if(!window.closed){
			nbLineStart=t.value.mb_substr(0, t.selectionStart).split("\n").length - 1;
			nbLineEnd=t.value.mb_substr(0, t.selectionEnd).split("\n").length - 1;
			try
			{
