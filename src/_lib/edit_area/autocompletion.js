...
var EditArea_autocompletion= {
	...
	,_select: function( content )
	{
		...
		line_string	= 	editArea.textarea.value.mb_substring( start_index, editArea.textarea.selectionEnd + 1);
		limit	= line_string.length -1;
		nbMatch	= 0;
		for( i =0; i<limit ; i++ )
		{
			if( line_string.mb_substring( limit - i - 1, limit ) == content.mb_substring( 0, i + 1 ) )
				nbMatch = i + 1;
		}
	...
	,_checkLetter: function(){
		...
		if( editArea.is_editable )
		{
			...
			{
				var last_chars	= str.mb_substring(Math.max(0, start-this.curr_syntax[i]["max_text_length"]), start);
				var matchNextletter	= str.mb_substring(start, start+1).match( this.curr_syntax[i]["match_next_letter"]);
					...
					if( match_word )
					{
						...
						//		parent.console.log( this.curr_syntax[i]["keywords"][prefix]['datas'][j]['is_typing'] );
							...
							//		parent.console.log('match');
									hasMatch = false;
									var before = last_chars.mb_substr( 0, last_chars.length - begin_word.length );
					...
					else if( this.forceDisplay || match_prefix_separator )
					{
						for(var prefix in this.curr_syntax[i]["keywords"])
						{
							for(var j=0; j<this.curr_syntax[i]["keywords"][prefix]['datas'].length; j++)
							{
								...
								{
									var before = last_chars; //.mb_substr( 0, last_chars.length );
									if( before.match( this.curr_syntax[i]["keywords"][prefix]['prefix_reg'] ) )
										hasMatch = true;
								}
