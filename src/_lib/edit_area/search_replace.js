	...
	EditArea.prototype.area_search= function(mode){
		...
		if(_$("area_search_reg_exp").checked){
			...
			pos= infos["full_text"].mb_substr(start).search(reg);
			pos_begin= infos["full_text"].search(reg);
			if(pos!=-1){
				pos+=start;
				length=infos["full_text"].mb_substr(start).match(reg)[0].length;
			}else if(pos_begin!=-1){
		...
		if(mode=="replace" && pos==infos["indexOfCursor"]){
			...
			if(_$("area_search_reg_exp").checked){
				...
				if(!_$("area_search_match_case").checked)
					opt+="i";
				var reg= new RegExp(search, opt);
				new_text= infos["full_text"].mb_substr(0, begin) + infos["full_text"].mb_substr(start).replace(reg, replace);
			}else{
				new_text= infos["full_text"].mb_substr(0, begin) + replace + infos["full_text"].mb_substr(begin + length);
			}
	...
	/*	this.area_select(0, 0);
		...
		if(_$("area_search_reg_exp").checked){
			...
			if(_$("area_search_match_case").checked){
				...
				while(pos!=-1){
					nb_change++;
					new_text+= this.textarea.value.mb_substring(start , pos)+replace;
					start=pos+ search.length;
					pos= lower_value.indexOf(lower_search, pos+1);
				}
				new_text+= this.textarea.value.mb_substring(start);
			}
		}
