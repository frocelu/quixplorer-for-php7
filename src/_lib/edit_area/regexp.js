...
/*	EditArea.prototype.htmlTag= function(v0, v1, v2, v3, v4,v5,v6,v7,v8,v9, v10){
		...
		if(tab.length>1){
			...
			for(i=1; i<tab.length-1; i++){
				...
				attributes+="<span class=attributeVal>"+tab[i].mb_substr(0,cut)+"</span>";
				attributes+="<span class=attribute>"+tab[i].mb_substr(cut)+"</span>=";
			}
	...
	*/
	...
	EditArea.prototype.colorize_text= function(text){
	  ...
	  /*
		...
		return text.mb_substr(1).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/µ_END_µ/g,"</span>").replace(/µ__([a-zA-Z0-9]+)__µ/g,"<span class='$1'>");
	};
