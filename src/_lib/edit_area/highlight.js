	...
	EditArea.prototype.maj_highlight= function(infos){
		...
		if(t.reload_highlight===true){
			...
			if(t.settings["debug"]){
				...
				debug_opti= ( doSyntaxOpti?"Optimisation": "No optimisation" )
					...
					//debug_opti+= "\nchanged: "+ infos["full_text"].mb_substring(ch.posStart, ch.posNewEnd);
					+ "\nchanged_line: "+ch.newTextLine
					+ "\nlast_changed_line: "+ch.lastTextLine
					+"\nstay_begin: "+ stay_begin.slice(-100)
					+"\nstay_end: "+ stay_end.mb_substr( 0, 100 );
					//debug_opti="start: "+stay_begin_len+ "("+nb_line_start_unchanged+") end: "+ (stay_end_len)+ "("+(splited.length-nb_line_end_unchanged)+") ";
					//debug_opti+="changed: "+ textToHighlight.mb_substring(stay_begin_len, textToHighlight.length-stay_end_len)+" \n";

					//debug_opti+="changed: "+ stay_begin.mb_substr(stay_begin.length-200)+ "----------"+ textToHighlight+"------------------"+ stay_end.mb_substr(0,200) +"\n";
					+"\n";
			}
		...
		if( doSyntaxOpti )
		{
			try
			{
				...
				replacedBloc		= t.last_hightlighted_text.mb_substring( stay_begin.length, t.last_hightlighted_text.length - stay_end.length );
				...
				// get the changes
				lastHtml	= replacedBloc.mb_substring( nbStart, lengthOld - nbEnd );
				newHtml		= updated_highlight.mb_substring( nbStart, lengthNew - nbEnd );
					...
					&& lastHtml.indexOf('<span') == -1 && lastHtml.indexOf('</span') == -1 )
				{
					...
					doHtmlOpti		= true;
					beginStr		= t.last_hightlighted_text.mb_substr( 0, stay_begin.length + nbStart );
					// fix special chars
					...
					else
					{
						...
						nbClosed = beginStr.mb_substr( Math.max( 0, beginStr.lastIndexOf( '<span', maxStartOffset - 1 ) ) ).split('</span').length - 1;
						while( nbClosed > 0 )
						{
							nbClosed--;
							parentSpan = parentSpan.parentNode;
					...
					else
					{
						...
						nbSubSpanBefore			= beginStr.mb_substr( lastEndPos ).split('<span').length-1;
					}
					...
					//nbUnchangedChars =  ? beginStr.length : beginStr.mb_substr( lastIndex + 1 ).replace( /&lt;/g, '<').replace( /&gt;/g, '>').replace( /&amp;/g, '&').length;
					...
					{
						nbUnchangedChars	= beginStr.mb_substr( lastIndex + 1 ).replace( /&lt;/g, '<').replace( /&gt;/g, '>').replace( /&amp;/g, '&').length;
						//nbUnchangedChars	+= beginStr.mb_substr( ).replace( /&/g, '&amp;').replace( /</g, '&lt;').replace( />/g, '&gt;').length - beginStr.length;
					}
				...
				//	console.log( textNode.data.replace(/&/g, '&amp;') );
					...
					if( t.isIE )
					{
						nbUnchangedChars	-= ( beginStr.mb_substr( beginStr.length - nbUnchangedChars ).split("\n").length - 1 );
