	...
	*/
	...
	set_base_url : function(){
		...
		if( !this.baseURL ){
			...
			for( i=0; i<elems.length; i++ ){
				if (elems[i].src && elems[i].src.match(/edit_area_[^\\\/]*$/i) ) {
					...
					src = src.mb_substring(0, src.lastIndexOf('/'));
					this.baseURL = src;
					this.file_name= elems[i].src.mb_substr(elems[i].src.lastIndexOf("/")+1);
					break;
				}
			}
		}
		...
		if (docBasePath.indexOf('?') != -1)
			docBasePath	= docBasePath.mb_substring(0, docBasePath.indexOf('?'));
		docBasePath	= docBasePath.mb_substring(0, docBasePath.lastIndexOf('/'));
    ...
    getSelectedText : function(id){
        ...
        return this.getValue(id).mb_substring(sel["start"], sel["end"]);
    },
	...
	setSelectedText : function(id, new_val){
		...
		text	= text.mb_substring(0, sel["start"])+ new_val +text.mb_substring(sel["end"]);
